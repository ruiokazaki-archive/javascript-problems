...
const d =
  ...
  u = ({ contentElement: r, problems: o, locationHref: n }) => {
    var i, s;
    const c =
      (s =
        (i = n || window.location.href) == null ? void 0 : i.split('#')[1]) ==
      null
        ? void 0
        : s.split('-');
    if (!c) return (r.innerHTML = d);
    const [e, t] = c;
    r.innerHTML = o[e][t] || '<h2>404 Not Found</h2>';
  },
  h = `
<h2>plus_one</h2>
...
`,
  f = { plus_one: h, maximum_product_of_three_numbers: m },
  y = ({ category: r, titles: o }) =>
    `<li class="nav-item"><span>${r}</span><ul>${o
      .map((n) => '<li><a href="#' + r + '-' + n + '">' + n + '</a></li>')
      .join()
      .replaceAll(',', '')}</ul></li>`,
  l = { array: f },
  a = document.getElementById('content');
window.addEventListener('load', () => {
  u({ contentElement: a, problems: l });
  const r = document.getElementById('categories');
  for (const o of Object.keys(l))
    r.innerHTML += y({ category: o, titles: Object.keys(l[o]) });
  document.querySelectorAll('a').forEach((o) =>
    o.addEventListener('click', (n) => {
      n.preventDefault();
      const c = n.target.href;
      u({ contentElement: a, problems: l, locationHref: c }),
